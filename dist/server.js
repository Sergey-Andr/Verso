...
async function loadEnvUniversal() {
    try {
        ...
        return (dir, dev) => loadEnvConfig(dir, dev);
    }
    catch {
        const mod = await import("@next/env");
        const fn = mod.loadEnvConfig ?? mod.default?.loadEnvConfig;
        if (!fn)
            throw new Error("@next/env: loadEnvConfig not found");
        return (dir, dev) => fn(dir, dev);
    }
}
...
async function start() {
    ...
    await (await loadEnvUniversal())(process.cwd(), dev);
    const port = Number(process.env.PORT || 8080);
    const hostname = "0.0.0.0";
    const WS_PATH = "/ws";
    const ALLOW = new Set((process.env.CORS_ORIGINS || "")
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean));
    ...
    if (!dev && handle) {
        ...
        app.get("/", (_req, res) => res
            .status(200)
            .send(`Dev WS server is up; Next dev runs on ${process.env.NEXT_PUBLIC_APP_PORT || 5174}`));
    }
    ...
    wss.on("connection", (ws) => {
        ws.on("message", (raw) => {
            ...
            if (msg?.type === "subscribe" && msg?.key) {
                ...
                (subscriptions.get(key) ?? subscriptions.set(key, new Set()).get(key)).add(ws);
                return;
            }
